...
#: 예시 표 상한. 프롬프트가 터지지 않을 만큼만 싣는다.
MAX_예시줄, MAX_예시칸, MAX_칸글자 = 30, 15, 200


def parse_table(글: str) -> list[list[str]]:
    """붙여넣은 예시 표를 줄×칸으로. 구분자는 알아서 고른다.

    엑셀에서 복사하면 **탭**으로 구분된 글자가 온다. 사람이 손으로 적을 때는
    `|` 를 쓰는 게 편하다. 둘 다 아니면 쉼표로 본다. 하나로 정해 두면
    "왜 안 되지" 가 되고, 그러면 이 기능을 안 쓴다.
    """
    줄들 = [l for l in (글 or "").replace("\r\n", "\n").split("\n") if l.strip()]
    if not 줄들:
        return []
    구분 = "\t" if any("\t" in l for l in 줄들) else ("|" if any("|" in l for l in 줄들)
                                                  else ",")
    표: list[list[str]] = []
    for 줄 in 줄들[:MAX_예시줄]:
        칸 = [c.strip()[:MAX_칸글자] for c in 줄.split(구분)][:MAX_예시칸]
        while 칸 and not 칸[-1]:          # 줄 끝의 빈 칸은 버린다 (| 로 감싼 경우)
            칸.pop()
        if 칸 and 칸[0] == "" and 구분 == "|":
            칸 = 칸[1:]                   # |로 시작한 줄
        if 칸:
...
def _예시블록(표: list[list[str]]) -> str:
    """예시 표를 프롬프트에 실을 모양으로."""
    if not 표:
        return ""
    그림 = "\n".join(" | ".join(칸) for 칸 in 표)
    return f"""
[예시 표]
사람이 "이런 모양으로 나왔으면 좋겠다" 고 붙여넣은 것이다.
- 첫 줄은 머리글로 본다. 머리글을 그대로 열 이름(머리글)으로 써라.
- 칸에 든 값은 **예시일 뿐이다.** 그대로 쓰지 마라. 어느 열에서 나온 값인지
  판단해서, **그 모양이 나오도록 수식**을 만들어라.
- 칸이 `홍길동(27세)` 이면
  `=한글_이름 & "(" & (YEAR(TODAY())-VALUE(LEFT(생년월일,4))) & "세)"`
- 칸이 `경)서울대 포닥 ('25.2~'26.8)` 처럼 여러 조각을 이어 붙인 것이면
  `&` 로 이어서 만들어라.
- 예시에 있는 사람 이름·학교 이름은 **그 사람만의 값**이다. 수식에 박아 넣지 마라.
...
def draft(설명: str, 아는열, llm: LLMClient | None = None, *,
          종류: str = "목록", 축목록=None,
          예시표: str | list | None = None) -> tuple[dict, list[str]]:
    ...
    """
    설명 = (설명 or "").strip()
    표 = parse_table(예시표) if isinstance(예시표, str) else (예시표 or [])
    if not 설명 and not 표:
        return {}, ["무엇을 만들지 적어 주세요. 예시 표를 붙여넣어도 됩니다."]
    if 종류 not in _모양:
    ...
    메모: list[str] = []
    if 표:
        메모.append(f"예시 표 {len(표)}줄 × {max(len(r) for r in 표)}칸을 보고 만들었습니다.")
