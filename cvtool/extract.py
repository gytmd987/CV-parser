...
- 현재_지도교수는 **한글명(원문명)** 형식이다. 한글로 적혀 있으면 한글명만
  (`이길호`), 영문이면 한글을 유추해 앞에 두고 원문을 괄호 안에
  (`Prof. Gil-Ho Lee` -> `이길호(Gil-Ho Lee)`). 직함(Prof./Dr.)은 뺀다.
  한글을 유추할 수 없으면 원문만 적고 지어내지 마라."""
...
- 지도교수 이름은 **한글명(원문명)** 형식이다. 한글로 적혀 있으면 한글명만
  (`이길호`), 영문이면 한글을 유추해 앞에 두고 원문을 괄호 안에
  (`Prof. Gil-Ho Lee` -> `이길호(Gil-Ho Lee)`). 직함(Prof./Dr.)은 뺀다.
  한글을 유추할 수 없으면 원문만 적고 지어내지 마라.
