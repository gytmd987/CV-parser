...
_PERMISSIONS: dict[str, tuple[str, ...]] = {
    ...
    # 지원자 상세의 **검토 필요 · 관리 정보** 카드. 현업은 자기 과제 지원자가
    # 어느 단계까지 왔는지만 보면 되는 사람이다. LLM 이 무엇을 확신하지 못했나,
    # 원본 파일명이 무엇인가, 언제 누가 등록했나는 채용을 굴리는 쪽의 일이다.
    "지원자_관리정보": ("관리자", "채용담당자"),
    # 과제 매칭은 **회사 연구 과제 전체**와 지원자 전원을 나란히 보여준다.
    # 현업에게는 자기 과제 밖의 지원자·과제가 그대로 노출되므로 뺀다.
    "과제매칭_조회": ("관리자", "채용담당자"),
    # 대시보드는 지원자 전원을 가로질러 세고 이름까지 뽑는다. 현업은 안 본다.
    "대시보드_조회": ("관리자", "채용담당자"),
    "명칭_관리": ("관리자", "채용담당자"),
    "부서과제_관리": ("관리자", "채용담당자"),
    "계정_현업추가": ("관리자", "채용담당자"),
