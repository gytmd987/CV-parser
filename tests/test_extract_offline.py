...
SECTION_REPLIES = {
    ...
    "research": {
        ...
        "특허": [
            # 같은 발명을 한국·미국에 등록했다. 국내 1 · 해외 1 로 세야 한다.
...
def test_paper_and_patent_counts():
    ...
    # 같은 제목을 한국·미국에 등록한 것이 국내 1 · 해외 1 이다 (2건이 아니다).
    # 출원 2건은 어느 열에도 안 들어간다.
    assert row["특허_등록_국내_수"] == "1" and row["특허_등록_해외_수"] == "1"
    assert "특허_출원_수" not in row


def test_patent_with_unknown_status_or_region_is_counted_in_neither():
    from cvtool.schemas import CVRecord, Patent

    rec = CVRecord(지원자_ID="X", 특허=[
        Patent(상태="불명", 국내해외="국내"),
        Patent(상태="등록", 국내해외="불명"),      # 나라를 모르면 안 센다
        Patent(제목="센다", 상태="등록", 국내해외="국내"),
    ])
    assert rec.특허_수() == {"특허_등록_국내_수": 1, "특허_등록_해외_수": 0}
