...
python3 -m pytest -q                       # LLM 없이 되는 검증 (96개)
...
| 무엇 | 어디 | 남는가 |
|---|---|---|
| **업로드한 CV 원본** | `~/.cvtool/incoming/<파일명>` | ❌ 텍스트 추출 직후 삭제 |
| CV 원문 텍스트 | `candidates.db` 의 `원문_텍스트` 열 | 기본 ❌ / `CVTOOL_STORE_CV_TEXT=1` 이면 ⭕ |
| **추출 결과(이름·연락처·이메일 포함)** | `~/.cvtool/candidates.db` | ⭕ 보관 기간까지 |
| 학회·저널 목록 | `~/.cvtool/venues.db` | ⭕ |
| 업로드 처리 현황 | 메모리만 | ❌ 재시작하면 사라짐 |

원본 파일은 추출하는 **몇 초 동안만** 디스크에 있다가 지워집니다. 추출이 실패해도
`finally` 에서 지웁니다. 프로세스가 강제 종료돼 파일이 남은 경우에는 **다음 기동
때 자동으로 삭제**하고 콘솔에 알립니다.

**파일 권한:** 디렉터리는 `0700`, DB 파일은 `0600` 으로 잠급니다. 개인정보가 든
DB 를 서버의 다른 계정이 읽으면 웹 로그인을 걸어둔 의미가 없기 때문입니다.
...
- ✅ 오프라인 테스트 96개 통과 (응답 정제, 섹션 분할, 학회 레지스트리, xlsx, 보관기간)
